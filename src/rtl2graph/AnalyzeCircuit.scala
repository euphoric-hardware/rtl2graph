...
case class NumMuxesAnnotation(message: String) extends NoTargetAnnotation

object AnalyzeCircuit extends Transform with DependencyAPIMigration {
  ...
  override def execute(state: CircuitState): CircuitState = {
    ...
    state.copy(annotations = state.annotations :+ NumMuxesAnnotation(ledger.serialize))
  }
